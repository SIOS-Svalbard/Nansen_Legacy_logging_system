...
def create_database(DB):
    ...
    if (DB['dbname'],) in list_databases:
        ...
        """
        init_institutions(DB, cur)
        init_stations(DB, cur)
        init_sample_types(DB, cur)
        init_gear_types(DB, cur)
        init_intended_methods(DB, cur)
        init_projects(DB, cur)
        init_personnel(DB, cur)
        init_storage_temperatures(DB, cur)
        init_filters(DB, cur)
        init_sex(DB, cur)
        init_kingdoms(DB, cur)
        init_user_field_setups(DB, cur)
...
def init_intended_methods(DB, cur):
...
def init_storage_temperatures(DB, cur):
...
def init_user_field_setups(DB, cur):
...
def create_cruise_tables(DB, METADATA_CATALOGUE, CRUISE_DETAILS_TABLE):
    '''
...
def run(DB, CRUISE_NUMBER, METADATA_CATALOGUE, CRUISE_DETAILS_TABLE):
    #create_database(DBNAME)
    create_database(DB)
    #create_cruise_tables(DBNAME, METADATA_CATALOGUE, CRUISE_DETAILS_TABLE)
    create_cruise_tables(DB, METADATA_CATALOGUE, CRUISE_DETAILS_TABLE)
